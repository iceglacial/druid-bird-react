...
const api = {
    ...
    movebankUser: (name) => `${base}/company/movebank/user/${name}`,//创建、修改movebank用户名 - put
    ...
    //新分页api
    gps2: (id, lastTime = '', begin, end) => {
        if (begin || end) {
            //return `${base}/gps/device/${id}/page/${lastTime}?${begin ? 'timestamp=' + begin : 'timestamp=' + end}`
            return `${base}/gps/device/${id}/page/${begin ? begin :  end}`
        } else {
            return `${base}/gps/device/${id}/page/${lastTime}`
        }
    },
    gps2Count: (id) => base + '/gps/device/' + id + '/count',
    //获取单个设备的 GPS 精简信息 - get
    gpsLine: (id) => `${base}/gps/device/${id}/line`,
    //新原始信息api
    gpsNewLine: (id, day) => `${base}/gps/device/${id}/last/${day}`,
    ...
    //新分页api
    behavior2: (id, lastTime = '', begin, end) => {
        if (begin || end) {
            // return `${base}/behavior/device/${id}/page/${lastTime}?${begin ? 'timestamp=' + begin : 'timestamp=' + end}`
            return `${base}/behavior/device/${id}/page/${begin ? begin : end}`

        } else {
            return `${base}/behavior/device/${id}/page/${lastTime}`
        }
    },
    behavior2Count: (id) => base + '/behavior/device/' + id + '/count',
    //behavior2新分页api
    behavior2_2: (id, lastTime = '', begin, end) => {
        if (begin || end) {
            //return `${base}/behavior2/device/${id}/page/${lastTime}?${begin ? 'timestamp=' + begin : 'timestamp=' + end}`
            return `${base}/behavior2/device/${id}/page/${begin ?  begin : end}`
        } else {
            return `${base}/behavior2/device/${id}/page/${lastTime}`
        }
    },
    behavior2_2Count: (id) => base + '/behavior2/device/' + id + '/count',
    ...
    //新分页api
    sms2: (id, lastTime = '', begin, end) => {
        if (begin || end) {
            //return `${base}/gps/device/${id}/sms/page/${lastTime}?${begin ? 'timestamp=' + begin : 'timestamp=' + end}`
            return `${base}/gps/device/${id}/sms/page/?${begin ?  begin : end}`
        } else {
            return `${base}/gps/device/${id}/sms/page/${lastTime}`
        }
    },
    sms2Count: (id) => `${base}/gps/device/${id}/sms/count`,
    ...
    *新数据分析
    */
    chartLast: (id, day) => `${base}/gps/device/${id}/last/${day}`,
    chartDay: (id, date) => `${base}/gps/device/${id}/day/${date}`,
    chartWeek: (id, date) => `${base}/gps/device/${id}/week/${date}`,
    chartMonth: (id, date) => `${base}/gps/device/${id}/month/${date}`,
    behavior2chartLast: (id, day) => `${base}/behavior2/device/${id}/last/${day}`,
    behavior2chartDay: (id, date) => `${base}/behavior2/device/${id}/day/${date}`,
    behavior2chartWeek: (id, date) => `${base}/behavior2/device/${id}/week/${date}`,
    behavior2chartMonth: (id, date) => `${base}/behavior2/device/${id}/month/${date}`,
}
export default api