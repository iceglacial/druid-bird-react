...
import {Api, DataFormatter, httpConfig, MessageHandle} from '../../../common/index'
import thisStore from './store'
import {observer} from 'mobx-react'
import ReactDataGrid from 'react-data-grid'
import appStore from '../../../store/app_store'
import {Pagination} from './detail_pagination'
import {DataGridColumns} from "../../../common";
...
class AppDevice extends React.Component {
    ...
    getCount = () => {
        axios.get(Api.sms2Count(this.props.deviceId), httpConfig()).then(res => {
            this.setState({
    ...
    loadPage = (p, direction, firstPage) => {
        ...
        /*
        //旧分页方式暂时封存
        axios.get(Api.sms(this.props.deviceId), config).then(res => {
          // #2466 错误时间的数据过滤
          ...
          thisStore.setSmsData(dataFormat('env',res.data));
        }).catch(err => {
          MessageHandle(err)
        })
        */
        ...
        axios.get(Api.sms2(this.props.deviceId, criticalTime, begin, end), config2).then(res => {
            if (needRevers) {
                res.data.reverse();
            }
            // #2466 错误时间的数据过滤
    ...
    isEnd() {
        return appStore.pageSize > this.state.totalLength;
    }
                    ...
                    <div className="wrap-full">
                        {
                            thisStore.loading ?
                                ...
                                <ReactDataGrid
                                    ...
                                    rowsCount={thisStore.smsData.length}
                                    minHeight={1000}
                                    rowHeight={45}
