...
const {Bhv2DataFormatter} = DataFormatter
...
class AppDevice extends React.Component {
    ...
    //     thisStore.setBehavior2Data(Bhv2DataFormatter(res.data));
    //   }).catch(err => {
    //     MessageHandle(err)
    //   })
    // }
    ...
    rowGetter(i) {
        return thisStore.behavior2Data[i];
    }
    ...
    getCount = () => {
        axios.get(Api.behavior2_2Count(this.props.deviceId), httpConfig()).then(res => {
            this.setState({
    ...
    loadPage = (p, direction, firstPage) => {
        ...
        */
        ...
        axios.get(Api.behavior2_2(this.props.deviceId, criticalTime, begin, end), config2).then(res => {
            if (needRevers) {
                res.data.reverse();
            }
            // #2466 错误时间的数据过滤
    ...
    getColumns = (_type, items) => {
        ...
        //todo 有空的话需要写活,(8.19添加了翻译)
        columns = [{key: "updated_at", name: appStore.language.uploading_time,},
            {key: "timestamp", name: appStore.language.acquisition_time,},
            {key: "odba", name: appStore.language.activity_index,}]
                    ...
                    <div className="wrap-full">
                        {
                            thisStore.loading ?
                                ...
                                <ReactDataGrid
                                    ...
                                    rowsCount={thisStore.behavior2Data.length}
                                    minHeight={1000}
                                    width={'100%'}
                                    rowHeight={45}
